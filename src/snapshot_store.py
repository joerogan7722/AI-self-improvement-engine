...
# Assuming Context is defined in src/core/role.py
from src.core.role import Context
...
class SnapshotStore:
    ...
    def record(self, context: Context):
        ...
        timestamp = context.metadata.get("timestamp", datetime.now().strftime("%Y%m%d_%H%M%S")) # Already done, no change needed
        snapshot_file_path = os.path.join(str(goal_snapshot_dir), f"{timestamp}.json") # Use os.path.join
